...
<head>
    ...
    <title>Contacto - Enviar Consulta</title>
...
<body>
    <div class="container mt-4 mb-4">
        <div class="d-flex justify-content-between align-items-center">
            <h1 class="text-primary">Contacto - Envía tu Consulta</h1>
            ...
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('guest.consultas.store') }}" method="POST">
                            @csrf

                            <div class="row g-3">
                                {{-- CAMPOS NUEVOS: Nombre, Apellido y Email del invitado --}}
                                <div class="col-md-6">
                                    <label for="nombre_guest" class="form-label">Tu Nombre</label>
                                    <input type="text" class="form-control @error('nombre_guest') is-invalid @enderror" id="nombre_guest" name="nombre_guest" value="{{ old('nombre_guest') }}" required>
                                    @error('nombre_guest')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6">
                                    <label for="apellido_guest" class="form-label">Tu Apellido</label>
                                    <input type="text" class="form-control @error('apellido_guest') is-invalid @enderror" id="apellido_guest" name="apellido_guest" value="{{ old('apellido_guest') }}" required>
                                    @error('apellido_guest')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6">
                                    <label for="email_guest" class="form-label">Tu Email</label>
                                    <input type="email" class="form-control @error('email_guest') is-invalid @enderror" id="email_guest" name="email_guest" value="{{ old('email_guest') }}" required>
                                    @error('email_guest')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
